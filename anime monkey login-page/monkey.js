...
function initLoginForm() {
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		const path = document.querySelector('svg path'); // Assuming only one path in SVG for simplicity
	
		emailInput.addEventListener('input', function() {
			if (emailInput.value.length > 0) {
				path.setAttribute('d', 'M50,140 Q100,' + (160 + emailInput.value.length) + ' 150,140');
			} else {
				path.setAttribute('d', 'M60,140 Q100,180 140,140');
			}
		});
	});
